...
class Supermarket {
    ...
    private int[][] T, M; // the complete weighted graph that measures the direct wheeling time to go from one point to another point in seconds
    ...
    private void backtracking(int u) {
        ...
        boolean isAllVisited = true, isStop = false;

        for (int i = 0; !isStop && i < shoppingList.length; i++) {
            if (isTest) {
                System.out.print("shoppingList[" + i + "]: " + shoppingList[i]);
                System.out.println(", visited[" + shoppingList[i] + "]: " + visited[shoppingList[i]]);
            }
            if (!visited[shoppingList[i]]) {
                isAllVisited = false;
                isStop = true;
            }
        }
