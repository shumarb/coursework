...
// write list of collaborators here: https://cpbook.net/#downloads, Ho Wei Chin
...
public class Bleeding {
    ...
    void modifiedDijkstra(int source){
        // Step 1: Do Modified Dijkstra Algo
        while(!pqueue.isEmpty()){

            // Step 1.1: Obtain first element in queue
            IntegerPair currPair = pqueue.poll();

            int currVert = currPair.second();
            int weight = currPair.first();
            if (test) {
                System.out.println("currVert = " + currVert + ", weight = " + weight);
            }

            // Step 1.2: Obtain neighbours of current vertex,
            ...
            while(iter.hasNext()) {
                ...
                boolean isRelax = relax(dist, v, currVert, weight_currVert_v);
                if (isRelax) {
    ...
    /**
     * Relax operation
     * Precon: There is at least 1 unvisited vertex
     * Postcon: Relax operation is successful
     */
    private boolean relax(ArrayList < Integer > dist, int v, int curr, int weight) {
        if(dist.get(v) > (dist.get(curr) + weight)) {
            dist.set(v, (dist.get(curr) + weight));
            return true;
        }
        return false;
    }
