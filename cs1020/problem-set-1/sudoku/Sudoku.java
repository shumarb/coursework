...
public class Sudoku {
    ...
    private boolean test = false, proceed = true;
    private int size, N;
    ...
    private boolean passRow(int[][]mat, int size){
        ...
        for(int a = 0; a<size && proceed == true; a++){
            int c = 0;
            int[] row = new int[size];
            for( int b = 0; b<size && proceed == true; b++){
                row[c] = mat[a][b];
                c+=1;
            }
    ...
    private boolean passSubMat(int[][] mat,int size){
        ...
        for(int a = 0; stop==false && a<size; a+=N){
            int start = a, endStart = start+N; // Condition to stop inner
            for(int b = 0; b<size && stop==false; b+=N){
                int end = b, endEnd = end+N;

                proceed = formSub(mat,start,endStart,end,endEnd);
                if( proceed==false ){
                    stop = true;
                    return false;
                }
    ...
    private void run(){
        ...
        if( proceed==false ){
            ...
            proceed  = passCol(mat,size);
            if( proceed==false ){
                printInvalid();
            }
            else{
                proceed = passSubMat(mat,size);
                if( proceed==false ){
                    printInvalid();
                }
                else{
